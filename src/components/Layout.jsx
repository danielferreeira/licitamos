import { LayoutDashboard, Gavel, Settings, LogOut, User, PieChart } from 'lucide-react'
import { useAuth } from '../contexts/AuthContext'

export function Layout({ children, onNavigate, currentPage }) {
  const { signOut, user } = useAuth()

  // Função para estilizar o link ativo vs inativo (Adaptada para Dark Mode)
  ...
  return (
    // Container Principal: Adicionado dark:bg-slate-950 e dark:text-slate-100
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-800 dark:text-slate-100 overflow-hidden transition-colors duration-300">
      ...
      {/* SIDEBAR: Adicionado dark:bg-slate-900 e dark:border-slate-800 */}
      <aside className="w-20 lg:w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-all duration-300 z-20 shadow-sm shrink-0">
        ...
        <div>
          {/* Header da Logo: Borda escura no dark mode */}
          <div className="p-6 flex items-center justify-center gap-3 border-b border-slate-50 dark:border-slate-800 h-24">
            
            {/* LOGO VERSÃO CLARA (Colorida) - Some no dark mode (dark:hidden) */}
            <img 
              src="/logo-full.png" 
              alt="Licitamos" 
              className="hidden lg:block dark:hidden h-12 object-contain"
              onError={(e) => {e.target.style.display='none'; document.getElementById('fallback-logo').style.display='flex'}}
            />

            {/* LOGO VERSÃO ESCURA (Branca) - Aparece só no dark mode (dark:lg:block) */}
            <img 
              src="/logo-full-dark.png" 
              alt="Licitamos Dark" 
              className="hidden dark:lg:block h-12 object-contain"
            />
            
            {/* Ícone Mobile (Mantemos o colorido por enquanto) */}
            <img 
              src="/logo-icon.png" 
              alt="Lc" 
          ...
          {/* Card de Usuário: Cores ajustadas para fundo escuro */}
          <div className="px-4 py-6 hidden lg:block">
            <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
              <div className="w-9 h-9 bg-white dark:bg-slate-700 rounded-full flex items-center justify-center text-brand-green border border-slate-200 dark:border-slate-600 shadow-sm">
                <User size={18} />
              </div>
              <div className="overflow-hidden">
                ...
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate w-32" title={user?.email}>
                  {userName}
                </p>
              </div>
            </div>
          ...
          <nav className="px-4 pb-4 space-y-2 mt-2">
            <a onClick={() => onNavigate('dashboard')} className={getLinkClass('dashboard')}>
              ...
              <span className="hidden lg:block">Visão Geral</span>
            </a>
            <a onClick={() => onNavigate('licitacoes')} className={getLinkClass('licitacoes')}>
              <Gavel size={20} />
              <span className="hidden lg:block">Licitações / Funil</span>
            </a>
        ...
        {/* Logout: Cores de hover ajustadas */}
        <div className="p-4 border-t border-slate-50 dark:border-slate-800 bg-white dark:bg-slate-900">
          <button 
            onClick={signOut}
            className="flex items-center gap-3 px-4 py-3 text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400 w-full rounded-xl transition-colors font-medium group"
          >
